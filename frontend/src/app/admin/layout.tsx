...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.get("/admin/organizations").then(({ data }) => {
      ...
      if (active) setError(e.response?.data?.error || "Verwaltung konnte nicht geladen werden.");
    });
    return () => { active = false; };
  }, []);
  ...
  return <>
    <header className="flex flex-col gap-3 border-b border-slate-700 bg-slate-950 px-4 py-4 text-white sm:px-6 lg:flex-row lg:items-center lg:gap-5">
      <Link href="/admin" className="font-bold">Relvona · Plattformverwaltung</Link>
      <label className="flex min-w-0 flex-col gap-1 text-sm sm:flex-row sm:items-center sm:gap-3">
        <span>Organisation</span>
        <select className="min-w-0 max-w-full rounded border border-slate-600 bg-slate-900 p-2" value={selected} onChange={(e) => { localStorage.setItem("active_org_id", e.target.value); setSelected(e.target.value); }}>
    ...
    {selected ? <div key={selected} className="min-w-0">{children}</div> : <p className="p-4 sm:p-10">Noch keine Organisation vorhanden. Erstellen Sie zunächst eine Organisation über die Registrierung.</p>}
  </>;
}
