...
export function BookingManager({ onError }: Props) {
  ...
  const [canRetryCalendarSync, setCanRetryCalendarSync] = useState(false);
  ...
  async function loadBookings(targetPage = page) {
    ...
    try {
      ...
      onError(error.response?.data?.error || "Buchungen konnten nicht geladen werden.");
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    void api.get("/scheduling/capabilities")
      .then(({ data }) => setCanRetryCalendarSync(Boolean(data.canRetryCalendarSync)))
      .catch(() => setCanRetryCalendarSync(false));
  }, []);
  ...
  async function loadSlots(booking: Booking) {
    ...
    try {
      ...
      setSlotsByBooking((current) => ({ ...current, [booking.id]: Array.isArray(data) ? data : [] }));
      setSelectedSlot((current) => ({ ...current, [booking.id]: "" }));
    } catch (error: any) {
      onError(error.response?.data?.error || "Freie Termine konnten nicht geladen werden.");
    } finally {
  ...
  async function reschedule(booking: Booking) {
    ...
    try {
      ...
      onError(error.response?.data?.error || "Termin konnte nicht umgebucht werden.");
  ...
  async function cancel(booking: Booking) {
    ...
    try {
      ...
      onError(error.response?.data?.error || "Termin konnte nicht storniert werden.");
    } finally {
      setMutatingId(null);
    }
  }
  ...
  async function retrySync(bookingId: string) {
    ...
    try {
      ...
      onError(error.response?.data?.error || "Kalender-Synchronisierung konnte nicht erneut gestartet werden.");
    } finally {
  ...
  async function toggleHistory(bookingId: string) {
    ...
    onError("");
    try {
      const { data } = await api.get(`/scheduling/bookings/${bookingId}/events`);
      setHistoryByBooking((current) => ({ ...current, [bookingId]: Array.isArray(data) ? data : [] }));
    } catch (error: any) {
      onError(error.response?.data?.error || "Booking-Historie konnte nicht geladen werden.");
    } finally {
  ...
  return <section className="rounded-xl border bg-card p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 className="font-semibold">Buchungen</h2><p className="mt-1 text-sm text-muted-foreground">Serverseitig filtern, über freie Slots umbuchen, stornieren und Audit-Historie prüfen.</p></div>
      <span className="text-xs text-muted-foreground">{total} Treffer</span>
    </div>
    ...
    <div className="mt-4 space-y-3">
      {loading ? <p className="text-sm text-muted-foreground">Buchungen werden geladen…</p> : bookings.length ? bookings.map((booking) => {
        const sync = booking.calendarSync;
        const retryable = canRetryCalendarSync && (sync?.status === "failed" || sync?.status === "exhausted");
        ...
        return <div key={booking.id} className="rounded-lg border p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><p className="font-medium">{new Date(booking.startsAt).toLocaleString("de-DE")}</p><p className="mt-1 text-xs text-muted-foreground">{booking.guestName || "Gast"} · {booking.guestEmail || "Ohne Gast-E-Mail"} · {booking.status}</p><p className="mt-1 text-xs text-muted-foreground">Booking-ID: {booking.id}</p></div>
            <div className="flex flex-wrap gap-2">
              {booking.meetingUrl && <a href={booking.meetingUrl} target="_blank" rel="noreferrer" className="rounded-lg border px-3 py-2 text-sm">Meeting öffnen</a>}
              <button type="button" disabled={historyLoadingId === booking.id} onClick={() => toggleHistory(booking.id)} className="rounded-lg border px-3 py-2 text-sm disabled:opacity-50">{historyLoadingId === booking.id ? "Historie lädt…" : history ? "Historie schließen" : "Historie"}</button>
              {active && <button type="button" disabled={loadingSlotsId === booking.id || mutatingId === booking.id} onClick={() => loadSlots(booking)} className="rounded-lg border px-3 py-2 text-sm disabled:opacity-50">{loadingSlotsId === booking.id ? "Slots laden…" : slots ? "Slots aktualisieren" : "Umbuchen"}</button>}
              {active && <button type="button" disabled={mutatingId === booking.id} onClick={() => cancel(booking)} className="rounded-lg border px-3 py-2 text-sm text-destructive disabled:opacity-50">{mutatingId === booking.id ? "Verarbeitet…" : "Stornieren"}</button>}
              {retryable && <button type="button" disabled={retryingBookingId === booking.id} onClick={() => retrySync(booking.id)} className="rounded-lg border px-3 py-2 text-sm disabled:opacity-50">{retryingBookingId === booking.id ? "Retry läuft…" : "Kalender-Sync erneut versuchen"}</button>}
            </div>
          </div>
      ...
      }) : <p className="text-sm text-muted-foreground">Keine Buchungen entsprechen den aktuellen Filtern.</p>}
    </div>
