...
export default function AdminIntegrationsPage() {
  ...
  async function createSyncRule() {
    ...
    try {
      const payload = syncMode === "create"
        ? { connectionId: syncConnectionId, eventType: "ticket.created", action: "zendesk.create_ticket" }
        : { connectionId: syncConnectionId, eventType: "ticket.updated", action: "zendesk.update_ticket" };
      await api.post("/integrations/sync/rules", payload);
      await load();
  ...
  async function retrySyncExecution(execution: SyncExecution) {
    setSyncLoading(true); setError("");
    try { await api.post(`/integrations/sync/executions/${execution.id}/retry`); await load(); }
    catch (e: any) { setError(e.response?.data?.error || "Sync-Ausführung konnte nicht erneut eingeplant werden."); }
    finally { setSyncLoading(false); }
  }
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-8">
    ...
    <section className="rounded-xl border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">Automatische Synchronisation</h2><p className="mt-1 text-sm text-muted-foreground">Neue Regeln sind immer deaktiviert. Create legt das externe Zendesk-Ticket an; Update synchronisiert nur bereits verknüpfte Tickets.</p></div><button onClick={() => load()} className="rounded-lg border px-3 py-1.5 text-xs">Aktualisieren</button></div><div className="mt-4 grid gap-3 md:grid-cols-[1fr_.8fr_auto]"><select value={syncConnectionId} onChange={(e) => setSyncConnectionId(e.target.value)} className="min-w-0 rounded-lg border bg-background px-3 py-2 text-sm"><option value="">Zendesk-Verbindung wählen</option>{zendeskConnections.map((connection) => <option key={connection.id} value={connection.id}>{connection.name}</option>)}</select><select value={syncMode} onChange={(e) => setSyncMode(e.target.value as SyncMode)} className="rounded-lg border bg-background px-3 py-2 text-sm"><option value="create">Ticket erstellen</option><option value="update">Status/Priorität aktualisieren</option></select><button disabled={!syncConnectionId || syncLoading} onClick={createSyncRule} className="rounded-lg bg-foreground px-4 py-2 text-sm text-background disabled:opacity-50">Regel anlegen</button></div><div className="mt-4 space-y-2">{syncRules.length ? syncRules.map((rule) => <div key={rule.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><p className="text-sm font-medium">{rule.eventType} → {rule.action}</p><p className="mt-1 text-xs text-muted-foreground">{connectionName(rule.connectionId)} · {rule.enabled ? "aktiv" : "deaktiviert"}</p></div><div className="flex gap-2"><button disabled={syncLoading} onClick={() => toggleSyncRule(rule)} className="rounded-lg border px-3 py-1.5 text-xs disabled:opacity-50">{rule.enabled ? "Deaktivieren" : "Aktivieren"}</button><button disabled={syncLoading} onClick={() => removeSyncRule(rule)} className="rounded-lg border px-3 py-1.5 text-xs text-destructive disabled:opacity-50">Löschen</button></div></div>) : <p className="text-sm text-muted-foreground">Noch keine automatischen Sync-Regeln angelegt.</p>}</div><div className="mt-5"><h3 className="text-sm font-medium">Letzte Sync-Ausführungen</h3><div className="mt-2 space-y-2">{syncExecutions.length ? syncExecutions.slice(0, 20).map((execution) => <div key={execution.id} className="grid gap-2 rounded-lg border p-3 text-xs md:grid-cols-[1fr_.7fr_.7fr_2fr_auto]"><div><p className="font-medium">{execution.eventKey}</p><p className="text-muted-foreground">{new Date(execution.createdAt).toLocaleString("de-DE")}</p></div><div><p className="text-muted-foreground">Status</p><p>{execution.status}</p></div><div><p className="text-muted-foreground">Extern</p><p>{execution.externalId || "–"}</p></div><div className="min-w-0"><p className="text-muted-foreground">Fehler</p><p className="break-words">{execution.error || "–"}</p></div><div className="flex items-center">{execution.status === "failed" && <button disabled={syncLoading} onClick={() => retrySyncExecution(execution)} className="rounded-lg border px-3 py-1.5 text-xs disabled:opacity-50">Erneut versuchen</button>}</div></div>) : <p className="text-sm text-muted-foreground">Noch keine Sync-Ausführungen vorhanden.</p>}</div></div></section>
