...
export default function InvitePage({
  ...
  useEffect(() => {
    void authClient
      .getSession()
      .then((result) => setHasSession(Boolean(result.data?.user)));
    params
      .then(({ token }) => {
        setToken(token);
      ...
      .catch((e) => setError(e.response?.data?.error || "Einladung ungültig."));
  }, [params]);
  const accept = async (event: React.FormEvent) => {
    event.preventDefault();
    try {
      await api.post(`/auth/invitations/${token}/accept`, { name, password });
  ...
  const acceptExisting = async () => {
    try {
      await api.post(`/auth/invitations/${token}/accept-existing`);
      window.location.assign("/");
    } catch (e: any) {
      setError(
        e.response?.data?.error || "Einladung konnte nicht angenommen werden.",
      );
    }
  };
  return (
    <main className="grid min-h-screen place-items-center bg-slate-950 p-6 text-white">
      {error ? (
        ...
        <section className="w-full max-w-md space-y-4 rounded-xl bg-slate-900 p-8">
          <h1 className="text-xl font-bold">Einladung annehmen</h1>
          <p className="text-sm text-slate-400">
            Angemeldet als vorhandenes Konto. Die E-Mail-Adresse muss mit{" "}
            {invite.email} übereinstimmen.
          </p>
          <button
            type="button"
        ...
        <form
          onSubmit={accept}
          className="w-full max-w-md space-y-4 rounded-xl bg-slate-900 p-8"
        >
          <h1 className="text-xl font-bold">Einladung annehmen</h1>
          <p className="text-sm text-slate-400">
