...
export default function NotificationCenter() {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => { if (event.key === "active_org_id") void load(); };
  ...
  return (
    <div className="fixed right-5 top-5 z-[60]">
      <button type="button" onClick={() => { setOpen((value) => !value); if (!open) void load(); }} className="relative grid h-11 w-11 place-items-center rounded-full border border-slate-700 bg-slate-950 text-slate-200 shadow-xl transition hover:bg-slate-900" aria-label="Benachrichtigungen">
      ...
      {open && (
        <div className="mt-2 w-[min(92vw,390px)] overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 text-slate-100 shadow-2xl">
          <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
            <div><h2 className="text-sm font-semibold">Benachrichtigungen</h2><p className="text-xs text-slate-500">{unread} ungelesen</p></div>
          ...
          <div className="max-h-[70vh] overflow-y-auto">
            ...
            {items.map((item) => (
              <button type="button" key={item.id} onClick={() => void markRead(item.id)} className={`block w-full border-b border-slate-900 px-4 py-3 text-left transition hover:bg-slate-900/70 ${item.readAt ? "opacity-60" : "bg-slate-900/30"}`}>
                <div className="flex items-start gap-3">
                  <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${item.severity === "warning" ? "bg-amber-400" : "bg-blue-400"}`} />
                  <div className="min-w-0"><p className="text-sm font-medium text-slate-100">{item.title}</p><p className="mt-1 text-xs leading-5 text-slate-400">{item.message}</p><p className="mt-1.5 text-[10px] text-slate-600">{new Date(item.createdAt).toLocaleString("de-DE")}</p></div>
                </div>
              </button>
            ))}
          </div>
