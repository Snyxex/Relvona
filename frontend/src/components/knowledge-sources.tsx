...
export default function KnowledgeSources({ sources, canManage, onRefresh, notify }: {
  ...
  return <section className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
    <div className="p-4 border-b border-slate-800 flex justify-between"><h3 className="font-semibold text-sm">Wissensquellen</h3><button type="button" className="text-xs text-blue-300" onClick={() => { void onRefresh(); }}>Aktualisieren</button></div>
    {editor && <form className="p-4 space-y-3 border-b border-slate-800" onSubmit={async (event) => {
      ...
      <label className="block text-xs">Titel / FAQ-Frage<input className="block w-full p-2 bg-slate-800 rounded mt-1" required maxLength={300} value={editor.title} onChange={(event) => setEditor({ ...editor, title: event.target.value })} /></label>
      <label className="block text-xs">Inhalt / FAQ-Antwort<textarea className="block w-full p-2 bg-slate-800 rounded mt-1" required rows={8} maxLength={500000} value={editor.content} onChange={(event) => setEditor({ ...editor, content: event.target.value })} /></label>
      <div className="flex gap-3"><label className="text-xs">Kategorie<input className="block p-2 bg-slate-800 rounded mt-1" maxLength={120} value={editor.category} onChange={(event) => setEditor({ ...editor, category: event.target.value })} /></label><label className="text-xs">Sprache<input className="block p-2 bg-slate-800 rounded mt-1" required pattern="[a-z]{2,3}(-[A-Za-z]{2,8})?" value={editor.language} onChange={(event) => setEditor({ ...editor, language: event.target.value })} /></label></div>
      <button type="submit" disabled={Boolean(busy)} className="rounded bg-blue-600 px-3 py-2 text-xs disabled:opacity-50">Speichern und neu verarbeiten</button><button type="button" className="ml-3 text-xs" onClick={() => setEditor(null)}>Abbrechen</button>
    </form>}
    <div className="overflow-x-auto"><table className="w-full text-left text-xs text-slate-300">
      ...
      <tbody className="divide-y divide-slate-800/60">{sources.map((source) => <tr key={source.id}>
        ...
        <td className="p-3"><span className={source.status === "failed" ? "text-red-300" : source.status === "completed" ? "text-emerald-300" : "text-amber-300"}>{source.status}</span>{source.job && <p className="mt-1 text-slate-500">Version {source.job.revision} · Versuch {source.job.attempts}/3</p>}{source.securityStatus !== "SAFE" && <p className="text-amber-300">Sicherheitsprüfung: {source.securityStatus} – nicht für KI-Antworten freigegeben</p>}{source.errorMessage && <p className="max-w-sm mt-1 text-red-300">{source.errorMessage}</p>}</td>
