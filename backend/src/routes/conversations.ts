...
router.put("/:id/assignment", async (req: AuthRequest, res) => { try { if (req.body?.agentId && req.body.agentId !== req.user!.id && !["owner", "admin"].includes(req.organization!.role)) return res.status(403).json({ error: "Only administrators can assign other agents" }); return res.json(await ConversationWorkflowService.assign({ organizationId: req.organization!.id, conversationId: req.params.id, actorUserId: req.user!.id, assigneeId: req.body?.agentId || null })); } catch (error) { return res.status(409).json({ error: (error as Error).message }); } });
