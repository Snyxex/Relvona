...
router.post("/message/stream", createRateLimiter({ keyPrefix: "widget-stream", limit: 20, windowMs: 60_000 }), validateInputLimits, async (req, res) => {
  ...
  try { setLogContext({ organizationId: assistant.organization_id, conversationId }); send("status", { state: "thinking" }); const result = await ConversationService.processCustomerMessage({ organizationId: assistant.organization_id, conversationId, content, onToken: async (token) => { send("token", { content: token }); } }); if (result.aiResponse?.content) send("complete", { messageId: result.aiResponse.id, content: result.aiResponse.content, state: result.state, handoffTriggered: result.handoffTriggered, ticketId: result.ticketId, ticketNumber: result.ticketNumber, ticketCreated: result.ticketCreated }); else { if (result.fallbackMessage) send("token", { content: result.fallbackMessage }); send("complete", { state: result.state, handoffTriggered: result.handoffTriggered, budgetFallback: result.budgetFallback, ticketId: result.ticketId, ticketNumber: result.ticketNumber, ticketCreated: result.ticketCreated }); } }
