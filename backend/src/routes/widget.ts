...
router.post("/message", createRateLimiter({ keyPrefix: "widget-message", limit: 20, windowMs: 60_000 }), validateInputLimits, async (req, res) => {
  try {
    ...
    setLogContext({ organizationId: assistant.organization_id, conversationId: conversation.id }); const result = await ConversationService.processCustomerMessage({ organizationId: assistant.organization_id, conversationId: conversation.id, content }); return res.json({ ...result, conversationToken: conversation.token });
...
router.post("/message/stream", createRateLimiter({ keyPrefix: "widget-stream", limit: 20, windowMs: 60_000 }), validateInputLimits, async (req, res) => {
  ...
  try {
    ...
    if (result.aiResponse?.content) {
      ...
      send("complete", { conversationId, conversationToken: conversation.token, customerId: result.customerMessage?.senderId || undefined, state: result.state, handoffTriggered: result.handoffTriggered, budgetFallback: result.budgetFallback, ticketId: result.ticketId, ticketNumber: result.ticketNumber, ticketCreated: result.ticketCreated });
    }
  }
...
router.post("/feedback", createRateLimiter({ keyPrefix: "widget-feedback", limit: 30, windowMs: 60_000 }), async (req, res) => {
  try {
    const { conversationId, conversationToken, messageId, rating, reason } = req.body || {}; const assistant = await validateConversationRequest(req);
    if (!assistant || !verifyWidgetSessionToken(conversationToken, { assistantId: assistant.id, organizationId: assistant.organization_id, conversationId })) return res.status(401).json({ error: "Invalid widget conversation session" });
    if (!messageId || ![-1, 1].includes(rating) || (reason !== undefined && (typeof reason !== "string" || reason.length > 500))) return res.status(400).json({ error: "Invalid feedback" });
