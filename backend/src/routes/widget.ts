...
async function conversationForMessage(assistant: PublicAssistant, body: any) {
  if (typeof body.conversationId === "string" && body.conversationId) return body.conversationId;
...
router.post("/message", createRateLimiter({ keyPrefix: "widget-message", limit: 20, windowMs: 60_000 }), validateInputLimits, async (req, res) => {
  try {
    ...
    const conversationId = await conversationForMessage(assistant, req.body);
...
router.post("/message/stream", createRateLimiter({ keyPrefix: "widget-stream", limit: 20, windowMs: 60_000 }), validateInputLimits, async (req, res) => {
  ...
  try {
    ...
    if (result.aiResponse?.content) {
      ...
      send("complete", { messageId: result.aiResponse.id, content: streamedContent ? undefined : result.aiResponse.content, conversationId, customerId: result.customerMessage.senderId || undefined, state: result.state, handoffTriggered: result.handoffTriggered, ticketId: result.ticketId, ticketNumber: result.ticketNumber, ticketCreated: result.ticketCreated });
    } else {
      if (result.fallbackMessage) send("token", { content: result.fallbackMessage });
      send("complete", { conversationId, customerId: result.customerMessage?.senderId || undefined, state: result.state, handoffTriggered: result.handoffTriggered, budgetFallback: result.budgetFallback, ticketId: result.ticketId, ticketNumber: result.ticketNumber, ticketCreated: result.ticketCreated });
    }
  }
