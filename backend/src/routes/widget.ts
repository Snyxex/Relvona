...
router.post("/message/stream", createRateLimiter({ keyPrefix: "widget-stream", limit: 20, windowMs: 60_000 }), validateInputLimits, async (req, res) => {
  ...
  try {
    ...
    if (result.aiResponse?.content) {
      ...
      send("complete", { messageId: result.aiResponse.id, content: streamedContent ? undefined : result.aiResponse.content, conversationId, customerId: result.customerMessage.senderId || undefined, state: result.state, handoffTriggered: result.handoffTriggered });
    } else {
      if (result.fallbackMessage) send("token", { content: result.fallbackMessage });
      send("complete", { conversationId, customerId: result.customerMessage?.senderId || undefined, state: result.state, handoffTriggered: result.handoffTriggered, budgetFallback: result.budgetFallback });
    }
  }
