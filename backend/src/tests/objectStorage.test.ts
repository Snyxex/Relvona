...
async function main() {
  const root = path.join(await mkdtemp(path.join(os.tmpdir(), "supportai-storage-parent-")), "objects");
  try {
    ...
    await storage.putObject(key, {
      body: Buffer.from("private tenant data"),
      contentLength: 19,
      contentType: "text/plain",
    });

    assert.equal(await storage.healthCheck(), "HEALTHY");
