...
export class SchedulingService {
  ...
  static async createMeetingType(data: {
    ...
    const [row] = await db.insert(meetingTypes).values({
      ...
      slug,
      description: data.description?.trim().slice(0, 2_000),
  ...
  static async createBooking(data: {
    ...
    const [conflict] = await db.select({ id: bookings.id }).from(bookings).where(and(
      ...
      lt(bookings.startsAt, conflictEnd),
      gt(bookings.endsAt, conflictStart),
    )).limit(1);
    ...
    const [booking] = await db.insert(bookings).values({
      ...
      guestEmail: data.guestEmail?.trim().toLowerCase().slice(0, 254),
      guestName: data.guestName?.trim().slice(0, 120),
      startsAt: data.startsAt,
      endsAt,
      timezone: data.timezone,
      idempotencyKey: data.idempotencyKey?.slice(0, 120),
      createdBy: data.createdBy?.slice(0, 40) || "system",
    }).returning();
