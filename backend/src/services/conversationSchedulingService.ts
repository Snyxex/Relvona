...
function selectedIndex(text: string, slots: Array<{ startsAt: string; timezone: string }>) {
  ...
  return slots.findIndex((slot) => { const parts = new Intl.DateTimeFormat("en-GB", { timeZone: slot.timezone, hour: "2-digit", minute: "2-digit", hour12: false }).formatToParts(new Date(slot.startsAt)); const h = Number(parts.find((part) => part.type === "hour")?.value); const m = Number(parts.find((part) => part.type === "minute")?.value); return h === hours && m === minutes; });
}
...
export class ConversationSchedulingService {
  static async handle(data: { organizationId: string; conversationId: string; customerId?: string; text: string }) {
    ...
    const enabled = (await SchedulingService.listMeetingTypes(data.organizationId)).filter((type) => type.enabled);
    ...
    const rules = await SchedulingService.listAvailabilityRules(data.organizationId, preferred.id);
    const candidateUserIds = [...new Set(rules.filter((rule) => rule.enabled && rule.userId).map((rule) => rule.userId!))];
    ...
    if (candidateUserIds.length) {
      for (const userId of candidateUserIds) { const candidate = await SchedulingService.findAvailableSlots({ organizationId: data.organizationId, meetingTypeId: preferred.id, assignedUserId: userId, from, to, limit: 3 }); if (candidate.length) { assignedUserId = userId; slots = candidate; break; } }
    } else {
      slots = await SchedulingService.findAvailableSlots({ organizationId: data.organizationId, meetingTypeId: preferred.id, from, to, limit: 3 });
    }
