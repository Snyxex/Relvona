...
export class ActionExecutionService {
  static async request(data: {
    ...
    const [execution] = await db.insert(actionExecutions).values({
      ...
      requiresApproval: String(tool.requiresApproval),
  ...
  static async list(organizationId: string, status?: string) {
    return db.select().from(actionExecutions)
      .where(and(eq(actionExecutions.organizationId, organizationId), status ? eq(actionExecutions.status, status) : undefined))
  ...
  static async approve(data: { organizationId: string; executionId: string; userId: string; reason?: string }) {
    const [updated] = await db.update(actionExecutions).set({
      status: "approved",
      approvedByUserId: data.userId,
      approvedAt: new Date(),
      decisionReason: data.reason?.slice(0, 1000),
      updatedAt: new Date(),
    }).where(and(
      eq(actionExecutions.organizationId, data.organizationId),
      eq(actionExecutions.id, data.executionId),
      eq(actionExecutions.status, "pending"),
      or(isNull(actionExecutions.expiresAt), gt(actionExecutions.expiresAt, new Date())),
    )).returning();
  ...
  static async reject(data: { organizationId: string; executionId: string; userId: string; reason?: string }) {
    const [updated] = await db.update(actionExecutions).set({
      status: "rejected",
      rejectedByUserId: data.userId,
      rejectedAt: new Date(),
      decisionReason: data.reason?.slice(0, 1000),
      updatedAt: new Date(),
    }).where(and(eq(actionExecutions.organizationId, data.organizationId), eq(actionExecutions.id, data.executionId), eq(actionExecutions.status, "pending"))).returning();
  ...
  static async execute(data: { organizationId: string; executionId: string; context: ToolExecutionContext }) {
    ...
    const [claimed] = await db.update(actionExecutions).set({ status: "executing", updatedAt: new Date() })
      .where(and(eq(actionExecutions.organizationId, data.organizationId), eq(actionExecutions.id, execution.id), eq(actionExecutions.status, "approved"))).returning();
    ...
    try {
      ...
      const [finished] = await db.update(actionExecutions).set({
        status: result.success ? "executed" : "failed",
        result: result.data || null,
        error: result.success ? null : (result.error || "Tool execution failed").slice(0, 2000),
        executedAt: new Date(),
        updatedAt: new Date(),
      }).where(and(eq(actionExecutions.organizationId, data.organizationId), eq(actionExecutions.id, execution.id))).returning();
      return finished;
    } catch (error) {
      const [failed] = await db.update(actionExecutions).set({
        status: "failed",
