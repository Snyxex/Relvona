...
export class CustomerPortalService {
  static async requestMagicLink(data: { organizationId: string; email: unknown }) {
    ...
    const result = await withTenantTransaction(data.organizationId, async (tx) => {
      ...
      if (!customer) {
        [customer] = await tx.insert(customers).values({ organizationId: data.organizationId, email, name: null }).returning();
      }
      ...
      if (!account) {
        [account] = await tx.insert(customerPortalAccounts).values({ organizationId: data.organizationId, customerId: customer.id }).returning();
      }
      ...
      await tx.insert(customerPortalMagicLinks).values({
    ...
    await CustomerPortalDeliveryService.sendMagicLink({
      organizationId: data.organizationId,
      email,
      purpose: result.purpose,
      magicLink: `${publicUrl}/verify?token=${encodeURIComponent(result.token)}`,
    });
  }
  ...
  static async consumeMagicLink(rawToken: unknown) {
    ...
    return withTenantTransaction(parsed.organizationId, async (tx) => {
      const [magicLink] = await tx.select().from(customerPortalMagicLinks)
        .where(and(
      ...
      const now = new Date();
      await tx.update(customerPortalMagicLinks).set({ usedAt: now })
        .where(and(eq(customerPortalMagicLinks.organizationId, parsed.organizationId), eq(customerPortalMagicLinks.id, magicLink.id)));
      await tx.update(customerPortalAccounts).set({ status: "active", emailVerifiedAt: account.emailVerifiedAt || now, lastLoginAt: now, updatedAt: now })
        .where(and(eq(customerPortalAccounts.organizationId, parsed.organizationId), eq(customerPortalAccounts.id, account.id)));
  ...
  static async authenticate(rawToken: unknown): Promise<CustomerPortalSessionContext> {
    ...
    return withTenantTransaction(parsed.organizationId, async (tx) => {
      const [row] = await tx.select({
        ...
        status: customerPortalAccounts.status,
      ...
      await tx.update(customerPortalSessions).set({ lastSeenAt: new Date() })
        .where(and(eq(customerPortalSessions.organizationId, parsed.organizationId), eq(customerPortalSessions.id, row.sessionId)));
  ...
  static async revokeSession(rawToken: unknown) {
    ...
    await withTenantTransaction(parsed.organizationId, async (tx) => {
      await tx.update(customerPortalSessions).set({ revokedAt: new Date() })
        .where(and(eq(customerPortalSessions.organizationId, parsed.organizationId), eq(customerPortalSessions.tokenHash, tokenHash(parsed.token))));
    });
  }
  ...
  static async linkVisitor(data: { session: CustomerPortalSessionContext; visitorToken: unknown; consentVersion: string }) {
    ...
    return withTenantTransaction(data.session.organizationId, async (tx) => {
      const [visitor] = await tx.select().from(anonymousVisitors)
        .where(and(eq(anonymousVisitors.organizationId, data.session.organizationId), eq(anonymousVisitors.visitorKeyHash, hash))).limit(1);
      if (!visitor) throw new Error("Visitor not found");
      const [link] = await tx.insert(customerPortalVisitorLinks).values({
        organizationId: data.session.organizationId,
        accountId: data.session.accountId,
        visitorId: visitor.id,
        consentVersion: data.consentVersion,
      }).onConflictDoUpdate({
        target: [customerPortalVisitorLinks.organizationId, customerPortalVisitorLinks.accountId, customerPortalVisitorLinks.visitorId],
        set: { consentVersion: data.consentVersion, consentedAt: new Date(), revokedAt: null },
      }).returning();
      return link;
    });
  }
  ...
  static async revokeVisitorLink(data: { session: CustomerPortalSessionContext; visitorId: string }) {
    await withTenantTransaction(data.session.organizationId, async (tx) => {
      await tx.update(customerPortalVisitorLinks).set({ revokedAt: new Date() })
        .where(and(
          eq(customerPortalVisitorLinks.organizationId, data.session.organizationId),
          eq(customerPortalVisitorLinks.accountId, data.session.accountId),
          eq(customerPortalVisitorLinks.visitorId, data.visitorId),
        ));
    });
  }
  ...
  static async dashboard(session: CustomerPortalSessionContext) {
    return withTenantTransaction(session.organizationId, async (tx) => {
      const [conversationRows, ticketRows, linkedVisitors] = await Promise.all([
        ...
        tx.select({ visitorId: customerPortalVisitorLinks.visitorId }).from(customerPortalVisitorLinks)
          .where(and(eq(customerPortalVisitorLinks.organizationId, session.organizationId), eq(customerPortalVisitorLinks.accountId, session.accountId), isNull(customerPortalVisitorLinks.revokedAt))),
      ]);
      const visitorIds = linkedVisitors.map((item) => item.visitorId);
      const linkedConversationRows = visitorIds.length
        ? await tx.select({ conversation: conversations }).from(visitorConversations)
            .innerJoin(conversations, eq(visitorConversations.conversationId, conversations.id))
            .where(and(eq(visitorConversations.organizationId, session.organizationId), eq(visitorConversations.visitorId, visitorIds[0])))
        : [];
      const memoryRows = visitorIds.length
        ? await tx.select().from(visitorMemories)
            .where(and(eq(visitorMemories.organizationId, session.organizationId), eq(visitorMemories.visitorId, visitorIds[0])))
        : [];
      ...
      return {
        ...
        memories: memoryRows.filter((memory) => memory.status === "active" && (!memory.expiresAt || memory.expiresAt > new Date())),
