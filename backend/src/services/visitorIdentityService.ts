...
export class VisitorIdentityService {
  ...
  static async memoriesForVisitor(organizationId: string, visitorId: string, limit = 20) {
    const [visitor] = await db.select({ memoryEnabled: anonymousVisitors.memoryEnabled })
      ...
      .where(and(eq(anonymousVisitors.organizationId, organizationId), eq(anonymousVisitors.id, visitorId)))
      .limit(1);
  ...
  static async conversationHistory(organizationId: string, visitorId: string, limit = 20) {
    return db.select({
      conversationId: visitorConversations.conversationId,
  ...
  static async createMemory(data: {
    ...
    const [memory] = await db.insert(visitorMemories).values({
  ...
  static async resolveMemory(data: { organizationId: string; visitorId: string; memoryId: string }) {
    const [memory] = await db.update(visitorMemories).set({ status: "resolved", updatedAt: new Date() })
      .where(and(eq(visitorMemories.organizationId, data.organizationId), eq(visitorMemories.visitorId, data.visitorId), eq(visitorMemories.id, data.memoryId)))
      .returning();
