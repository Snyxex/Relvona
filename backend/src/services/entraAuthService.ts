...
export class EntraAuthService {
  static async startConfiguration(organizationId: string) {
    ...
    if (!record || record.organizationStatus !== "active" || !record.ssoEnabled || !record.tenantId || !record.clientId) {
      throw new Error("SSO_NOT_ENABLED");
    }
  ...
  /** Atomically consumes an SSO state before any external token request. */
  static async consumeLoginState(stateHash: string): Promise<EntraLoginContext> {
    ...
    if (!record || record.organizationStatus !== "active" || !record.ssoEnabled || !record.tenantId || !record.clientId || !record.clientSecretEncrypted) {
      throw new Error("SSO_NOT_ENABLED");
    }
  ...
  static async resolveUserAndMembership(input: {
    ...
    return db.transaction(async (tx) => {
      ...
      if (!userId) {
        ...
        if (existingUser) {
          const [membership] = await tx.select({ id: organizationMembers.id }).from(organizationMembers)
            .where(and(eq(organizationMembers.organizationId, input.organizationId), eq(organizationMembers.userId, existingUser.id), eq(organizationMembers.status, "active")))
            .limit(1);
      ...
      const [membership] = await tx.select({ id: organizationMembers.id, status: organizationMembers.status }).from(organizationMembers)
        .where(and(eq(organizationMembers.organizationId, input.organizationId), eq(organizationMembers.userId, userId)))
        .limit(1);
      ...
      if (!membership) {
        ...
        if (invitation) {
          ...
          await tx.update(organizationInvitations).set({ acceptedAt: new Date(), updatedAt: new Date() })
