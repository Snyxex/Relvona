...
export class MicrosoftCalendarProvider implements CalendarProvider {
  ...
  private async accessToken() {
    ...
    const response = await fetch(`https://login.microsoftonline.com/${encodeURIComponent(tenant)}/oauth2/v2.0/token`, {
      ...
      body: new URLSearchParams({ client_id: clientId, client_secret: clientSecret, refresh_token: this.credentials.refreshToken, grant_type: "refresh_token", scope: "offline_access Calendars.ReadWrite User.Read" }),
    ...
    this.credentials = { ...this.credentials, accessToken: body.access_token, refreshToken: body.refresh_token || this.credentials.refreshToken, expiresAt: Date.now() + Math.max(60, body.expires_in || 3600) * 1000 };
    await this.onTokenUpdate?.(this.credentials);
    return this.credentials.accessToken;
  }

  private basePath() { return this.credentials.userId ? `/users/${encodeURIComponent(this.credentials.userId)}` : "/me"; }
  ...
  async listBusyIntervals(data: { from: Date; to: Date }): Promise<CalendarBusyInterval[]> {
    const schedule = this.credentials.email;
    if (!schedule) throw new Error("Microsoft Calendar connection is missing the mailbox email");
    const result = await this.request(`${this.basePath()}/calendar/getSchedule`, {
      method: "POST",
      headers: { Prefer: 'outlook.timezone="UTC"' },
      body: JSON.stringify({ schedules: [schedule], startTime: { dateTime: data.from.toISOString().replace(/Z$/, ""), timeZone: "UTC" }, endTime: { dateTime: data.to.toISOString().replace(/Z$/, ""), timeZone: "UTC" }, availabilityViewInterval: 30 }),
  ...
  async createEvent(input: CalendarEventInput): Promise<CalendarEventResult> {
    ...
    const result = await this.request(target, { method: "POST", body: JSON.stringify({ subject: input.title, body: { contentType: "text", content: input.description || "" }, start: { dateTime: input.start.toISOString().replace(/Z$/, ""), timeZone: "UTC" }, end: { dateTime: input.end.toISOString().replace(/Z$/, ""), timeZone: "UTC" }, attendees: input.attendeeEmail ? [{ emailAddress: { address: input.attendeeEmail, name: input.attendeeName }, type: "required" }] : [], isOnlineMeeting: true, onlineMeetingProvider: "teamsForBusiness" }) }) as any;
  ...
  async updateEvent(externalEventId: string, input: CalendarEventInput): Promise<CalendarEventResult> {
    const target = this.credentials.calendarId ? `${this.basePath()}/calendars/${encodeURIComponent(this.credentials.calendarId)}/events/${encodeURIComponent(externalEventId)}` : `${this.basePath()}/events/${encodeURIComponent(externalEventId)}`;
    const result = await this.request(target, { method: "PATCH", body: JSON.stringify({ subject: input.title, body: { contentType: "text", content: input.description || "" }, start: { dateTime: input.start.toISOString().replace(/Z$/, ""), timeZone: "UTC" }, end: { dateTime: input.end.toISOString().replace(/Z$/, ""), timeZone: "UTC" }, attendees: input.attendeeEmail ? [{ emailAddress: { address: input.attendeeEmail, name: input.attendeeName }, type: "required" }] : [] }) }) as any;
